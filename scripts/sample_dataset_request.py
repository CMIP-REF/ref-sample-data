...
class DataRequest(ABC):
    ...
    def create_out_filename(self, metadata: pd.Series, ds: xr.Dataset) -> pathlib.Path:
...
def create_sample_dataset(DataRequest):
    ...
    for _, dataset in datasets.iterrows():
        for ds_filename in dataset["files"]:
            ...
            output_filename = OUTPUT_PATH / DataRequest.create_out_filename(dataset, ds_decimated)
...
if __name__ == "__main__":
    ...
    for facets in facets_to_fetch:
        CMIP6DataRequest = CMIP6Request(
            facets={x: facets[x] for x in facet_id_keys},
            remove_ensembles=facets["remove_ensembles"],
            time_span=facets["time_span"],
        )
        create_sample_dataset(CMIP6DataRequest)
